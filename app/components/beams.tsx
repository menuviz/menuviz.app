...
// Ported from React Bits' <Beams /> (JS + CSS variant): a raymarched-noise
// plane stack lit by a single directional light, used as a decorative WebGL
// backdrop. Kept close to the upstream source — only the hardcoded black
...
interface ExtendMaterialConfig {
  ...
  vertex?: ShaderChunkMap;
  fragment?: ShaderChunkMap;
...
const CanvasWrapper = ({ children, className }: { children: React.ReactNode; className?: string }) => (
...
export interface BeamsProps {
  ...
  scale?: number;
  rotation?: number;
  /** Scene background color — matches the card surface instead of upstream's hardcoded black. */
  backgroundColor?: string;
  /** Base material color under the light — upstream hardcodes this to black. */
  surfaceColor?: string;
  roughness?: number;
  metalness?: number;
  envMapIntensity?: number;
  ...
  /** Camera distance from the beam field — lower is more "zoomed in". */
...
const Beams = ({
  ...
  vec3 getNormal(vec3 pos) {
    ...
    gl_FragColor.rgb -= randomNoise / 15. * uNoiseIntensity;`,
        ...
        material: { fog: true },
        uniforms: {
          diffuse: new THREE.Color(...hexToNormalizedRGB(surfaceColor)),
    ...
    [speed, noiseIntensity, scale, surfaceColor, roughness, metalness, envMapIntensity]
  );
  ...
  return (
    <CanvasWrapper className={className}>
      <group rotation={[0, 0, THREE.MathUtils.degToRad(rotation)]}>
        ...
        <DirLight color={lightColor} intensity={directionalIntensity} position={[0, 3, 10]} />
      </group>
